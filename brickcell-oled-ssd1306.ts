...
namespace Brickcell {
    //% color="red" icon="\uf12e" weight=70
    export namespace oled_ssd1306 {
        let font: number[] = [];
        font[0] = 0x0022d422;
        font[1] = 0x0022d422;
        font[2] = 0x0022d422;
         ...
         * @param color is line color, eg: 1
         */
        //% blockId="brickcell_oled_ssd1306_drawVLine"
        //% block="draw a vertical line at x %x|y %y|number %len|color %color"
        //% subcategory="oled_ssd1306"
        export function drawVLine(x: number, y: number, len: number, color: number = 1) {
         ...
         * @param color is line color, eg: 1
         */
        //% blockId="brickcell_oled_ssd1306_drawRect"
        //% block="draw a rectangle at |x1 %x1 y1 %y1|x2 %x2 y2 %y2|with color %color"
        //% parts=OLED12864_I2C trackArgs=0
        //% subcategory="oled_ssd1306"
        export function drawRect(x1: number, y1: number, x2: number, y2: number, color: number = 1) {
            if (x1 > x2)
                x1 = [x2, x2 = x1][0];
            if (y1 > y2)
                y1 = [y2, y2 = y1][0];
         ...
         */
        //% blockId="brickcell_oled_ssd1306_invertOLED"
        //% block="invert oled display %d"
        //% subcategory="oled_ssd1306"
        export function invertOLED(d: boolean = true) {
            let n = (d) ? 0xA7 : 0xA6
            cmd1(n)
        }
         ...
         */
        //% blockId="brickcell_oled_ssd1306_renderOLED"
        //% block="renderOLED"
        //% subcategory="oled_ssd1306"
        export function renderOLED() {
            set_pos()
         ...
         * clear screen
         */
        //% blockId="brickcell_oled_ssd1306_clearOLED"
        //% block="clearOLED"
        //% subcategory="oled_ssd1306"
         ...
         * turn on screen
         */
        //% blockId="brickcell_oled_ssd1306_onOLED"
        //% block="turn on OLED"
        //% subcategory="oled_ssd1306"
        export function onOLED() {
            cmd1(0xAF)
        }
         ...
         * turn off screen
         */
        //% blockId="brickcell_oled_ssd1306_offOLED"
        //% block="turn off OLED"
        //% subcategory="oled_ssd1306"
        export function offOLED() {
            cmd1(0xAE)
        }
         ...
         */
        //% blockId="brickcell_oled_ssd1306_zoomMode"
        //% block="zoom mode %d"
        //% subcategory="oled_ssd1306"
        export function zoomMode(d: boolean = true) {
            _ZOOM = (d) ? 1 : 0
            cmd2(0xd6, _ZOOM)
        }
         ...
         */
        //% blockId="brickcell_oled_ssd1306_initOLED"
        //% block="init OLED with addr %addr"
        //% subcategory="oled_ssd1306"
        export function initOLED(addr: number) {
            _I2CAddr = addr;
